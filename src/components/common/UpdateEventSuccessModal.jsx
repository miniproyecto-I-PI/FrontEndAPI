...
/**
 * UpdateEventSuccessModal.jsx
 * Espejo de CreateEventSuccessModal, para el flujo de edición (US-03).
 */
export default function UpdateEventSuccessModal({
  eventName,
  onStay,
  onGoToEvents,
}) {
  ...
  function handleGoToEvents() {
    onGoToEvents?.();
    navigate("/eventos", { state: { toast: "Cambios guardados" } });
  }
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="update-event-success-title"
      ...
      <div className="relative w-full max-w-lg bg-paper-card border border-sepia-border rounded-sharp p-6 md:p-8 shadow-xl warm-card-shadow text-center space-y-5">
        ...
        <div className="space-y-2">
          ...
          <p className="font-body text-xs md:text-sm text-ink-muted leading-relaxed max-w-md mx-auto">
            ...
            han sido guardados en tu bitácora.
          </p>
        </div>
